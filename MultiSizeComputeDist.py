...
for ksize in range(len(sizes)):
    ...
    for ii in range(cells_per_proc):
        ...
        new_zmin, new_zmax     = fz.get_new_zmin_zmax([nH[index], nC[index]], [xHp[index], xCp[index]], temp[index], grain_size, Ntot[index], grain_type, Qabs, zeta, zeq=zeq, G0=G0, includeCR=True)
